  ...
  uint16_T msgType, uint16_T isAcceptanceFilteringEnabled, uint32_T msgIDMask)
{
  ...
  if (isAcceptanceFilteringEnabled == 1) {
    mbx->MSGID.bit.AME = 1;            // Flag for filtering
    volatile union CANLAM_REG *lam = &ECanaLAMRegs.LAM0 + mbxNo;
    if (msgType==1) {
      lam->all = ~msgIDMask;           // message ID mask
    } else {
      lam->all = ~(msgIDMask << 18);   // message ID mask
    }

    lam->bit.LAMI = 0;
           // set 0 to accept messages depending on receive mailbox message type
  } else {
    mbx->MSGID.bit.AME = 0;
  }
